...
static void process_image(const void *p, int size)
{
    ...
    double diff = (double)(frame_time.tv_sec - fps_start_time.tv_sec) + (double)(frame_time.tv_nsec - fps_start_time.tv_nsec) / 1e9;
    fps_frame_count++;

    printf("diff : %f", diff);
    printf("fps_frame_count : %d", fps_frame_count);

    if (diff >= 60.0)
    {
	    double average_fps = fps_frame_count / diff;

            printf("avg fps over : %f", diff);
            printf("sec : %f", average_fps);
...
static void mainloop(void)
{
    ...
    while (count > 0)
    {
	    ...
	    diff = (double)(frame_time.tv_sec - fps_start_time.tv_sec) + (double)(frame_time.tv_nsec - fps_start_time.tv_nsec) / 1e9;
	    fps_frame_count++;
